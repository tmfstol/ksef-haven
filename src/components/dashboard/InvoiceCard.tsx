...
export function InvoiceCard({ invoice, isNew }: InvoiceCardProps) {
  ...
  return (
    <div className={`glass-panel-elevated rounded-xl p-4 ${isNew ? "border-l-2 border-l-primary" : ""}`}>
      ...
      {invoice.ksef_number && (
        <div className="flex gap-1.5">
          <Button variant="ghost" size="sm" className="flex-1 h-8 text-xs rounded-lg gap-1" disabled={!!downloading} onClick={() => handleDownload("xml")}>
            {downloading === "xml" ? <Loader2 className="h-3 w-3 animate-spin" /> : <FileCode className="h-3 w-3" />}
            XML
          </Button>
          <Button variant="ghost" size="sm" className="flex-1 h-8 text-xs rounded-lg gap-1" disabled={!!downloading} onClick={() => handleDownload("pdf")}>
            {downloading === "pdf" ? <Loader2 className="h-3 w-3 animate-spin" /> : <FileText className="h-3 w-3" />}
            PDF
          </Button>
        </div>
      )}
