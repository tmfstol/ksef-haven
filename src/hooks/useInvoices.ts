...
export function useSync(companyId?: string | null) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Auto-trigger biała lista VAT check for new invoices (background)
      if (newCount > 0 && companyId) {
        supabase.functions.invoke("vat-whitelist-check", {
          body: { company_id: companyId, only_unchecked: true },
        }).then(() => {
          queryClient.invalidateQueries({ queryKey: ["invoices"] });
        }).catch((e) => console.warn("VAT whitelist check failed:", e));
      }
    },
    onError: (error: Error) => {
